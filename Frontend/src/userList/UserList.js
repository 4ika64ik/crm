...
const UserList = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://construction-build.com:8081/users');
  ...
  const handleCall = async (userId) => {
    try {
      await axios.put(`http://construction-build.com:8081/update/${userId}`, { status: 1 });
  ...
  const handleNoAnswer = async (userId) => {
    try {
      await axios.put(`http://construction-build.com:8081/update/${userId}`, { status: 0 });
